CREATE OR REPLACE FUNCTION get_event_data(
    ...
    function_id_param INT default 1,
    time_from_param INT default 1,
    time_to_param INT default 1
)
RETURNS TABLE (
    o_lg_id INTeger,
...
WHERE vals.dim_id = ANY(dim_id_param)
            GROUP BY 1
            ORDER BY 1
        ),
        allowed_metrics AS (
        ...
        base AS (
            SELECT 
                ...
                en_id,
                CASE
                    WHEN 1 = 1 THEN date_trunc('minute', ev_ts)
                    WHEN 1 = 2 THEN date_trunc('hour', ev_ts)
                    WHEN 1 = 3 THEN date_trunc('day', ev_ts)
                    WHEN 1 = 4 THEN date_trunc('month', ev_ts)
